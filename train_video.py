...
class DefTrainer2d:
    """Train 2d gaussians to fit frame_i."""
    def __init__(
        self,
        image_path: Path,
        num_points: int = 2000,
        ...
        # match = re.search(r'\d+', self.image_name);self.t = int(match.group())
        image_name = self.image_name.split(".")[0]# r_23
        self.t = image_name.split("_")[1]# 23
        fid = int(self.t)/199
        # 2DGS
        if model_name == "GaussianImage_Cholesky":
            from gaussianvideo_cholesky import GaussianVideo_Cholesky
            self.gaussian_model = GaussianVideo_Cholesky(t=self.t, loss_type="L2", opt_type="adan", num_points=self.num_points, H=self.H, W=self.W, BLOCK_H=BLOCK_H, BLOCK_W=BLOCK_W, 
    ...
    def train(self):     
        ...
        for iter in range(1, self.iterations+1):
            # ---------------------------------------
            if iter < 10_000:  # warm_up = 10_000
                d_xyz, d_cholesky = 0.0, 0.0, 0.0
            else:
                time_input = self.fid.unsqueeze(0).expand(self.gaussian_model.get_xyz.shape[0], -1)
                d_xyz, d_cholesky = self.deform_model.step(self.gaussian_model.get_xyz.detach(), time_input)
                # ast_noise = 0 if dataset.is_blender else torch.randn(1, 1, device='cuda').expand(N,-1) * time_interval * smooth_term(iteration)
                # d_xyz, d_rotation, d_scaling = deform.step(self.gaussian_model.get_xyz.detach(), time_input + ast_noise)
            # ---------------------------------------
            # loss, psnr = self.gaussian_model.train_iter(self.gt_image, iter)
            loss, psnr = self.gaussian_model.train_iter(self.gt_image, iter, d_xyz, d_cholesky)
        ...
        self.deform.save_weights(model_path, self.iterations)
...
def main(argv):
    ...
    if args.data_name == "synthetic": # test-set
        image_length, start = 200, 0

    # warm-up: r_0 for 10k
    image_path = Path(args.dataset) / 'r_0.png'# TODO: or. all imgs?
    trainer = SimpleTrainer2d(image_path=image_path, num_points=args.num_points,
                           iterations=10000, model_name=args.model_name, args=args,
                           model_path=args.model_path)
    psnr, ms_ssim, training_time, eval_time, eval_fps = trainer.train()
    image_h, image_w = trainer.H, trainer.W
    logwriter.write("Warmup: {}x{}, PSNR:{:.4f}, MS-SSIM:{:.4f}, Training:{:.4f}s, Eval:{:.8f}s, FPS:{:.4f}".format(
        image_h, image_w, psnr, ms_ssim, training_time, eval_time, eval_fps))

    # deform-NN: r0 ~r199 for 50k
    image_h, image_w = 800, 800
    frames = list(range(image_length))
    random.shuffle(frames)
    frame_stack = frames.copy()
    for iteration in range(0, args.iterations + 1):# 单图 -> 1 iter
        # 所有图片，共用一个gaussian，deformable
        if not frame_stack:
            frame_stack = frames.copy()
        idx = frame_stack.pop(randint(0, len(frame_stack)-1))# 随机弹出一张: r_idx
        image_path = Path(args.dataset) / f'r_{idx}.png'
        trainer = DefTrainer2d(image_path=image_path, num_points=args.num_points,
            iterations=100, model_name=args.model_name, args=args, model_path=args.model_path)# 每张过100 iter 
        psnr, ms_ssim, training_time, eval_time, eval_fps = trainer.train()
        psnrs.append(psnr)
