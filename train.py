...
class SimpleTrainer2d:
    ...
    def __init__(
        ...
        self.log_dir = Path(
            f"./checkpoints/{args.data_name}/{model_name}_{args.iterations}_{num_points}/{self.image_name}")

        if model_name == "GaussianImage_Cholesky":
            from gaussianimage_cholesky import GaussianImage_Cholesky
            self.gaussian_model = GaussianImage_Cholesky(loss_type="L2", opt_type="adan", num_points=self.num_points,
...
def main(argv):
    ...
    for i in range(start, start + image_length):
        if args.data_name == "kodak":
            ...
            image_path = Path(args.dataset) / f'r_{i}.png'
